function lgraph = createUnet(imageSize,dropout_prob,encoderDepth)

% EDIT: modify these parameters for your application.
% encoderDepth = 4;
initialEncoderNumChannels = 32;
inputTileSize = [imageSize 3];
convFilterSize = [3 3];
inputNumChannels = 3;
numClasses = 2;
...
[encoder, finalNumChannels] = iCreateEncoder(encoderDepth, convFilterSize, initialEncoderNumChannels, inputNumChannels,dropout_prob);
...
dropOutLayer = dropoutLayer(dropout_prob,'Name','Bridge-DropOut');
...
%--------------------------------------------------------------------------
function [encoder, finalNumChannels] = iCreateEncoder(encoderDepth, convFilterSize, initialEncoderNumChannels, inputNumChannels,dropout_prob)
...
for stage = 1:encoderDepth
    ...
    if stage == encoderDepth
        dropOutLayer = dropoutLayer(dropout_prob,'Name',['Encoder-Stage-' num2str(stage) '-DropOut']);
